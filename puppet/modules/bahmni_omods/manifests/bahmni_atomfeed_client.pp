define bahmni_omods::bahmni_atomfeed_client($atomfeed_client_name) {
    ...
    $atomfeed_client_omod_name = "${atomfeed_client_name}-atomfeed-client-${::config::bahmni_release_version}"
    ...
    exec { "explode_${atomfeed_client_name}_atomfeed_jar" :
      command     => "unzip -o -d ${temp_dir}/${atomfeed_client_omod_name} ${openmrs_modules_dir}/${atomfeed_client_omod_name}.omod   ${::config::deployment_log_expression}",
      path        => "${config::os_path}",
      provider    => shell,
    ...
    exec { "run_${atomfeed_client_name}_atomfeed_client_liquibase" :
      command     => "${temp_dir}/run-${atomfeed_client_name}-atomfeed-client-liquibase.sh   ${::config::deployment_log_expression}",
      path        => "${config::os_path}",
      provider    => shell,

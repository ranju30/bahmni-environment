class bahmni_jasperreports inherits bahmni_jasperreports::config {
  ...
  file { "delete_reports_zip" :
    path      => "${::config::build_output_dir}/${::config::implementation_name}-reports.zip",
    ensure    => absent,
    force     => true,
  }

  exec { "download_reports_zip":
    command => "/usr/bin/wget --no-check-certificate ${config::report_zip_source_url} -O ${::config::implementation_name}-reports.zip",
  ...
  exec { "unzip_report" :
    command   => "unzip -q -o ${::config::implementation_name}-reports.zip -d ${::config::build_output_dir}/${::config::implementation_name}-reports   ${::config::deployment_log_expression}",
    provider  => shell,
    path      => "${config::os_path}",
    cwd       => "${::config::build_output_dir}",
  ...
  exec { "change-reports-name":
    command   => "mv *-master ${::config::implementation_name}-reports-master",
    path      => "${config::os_path}",
    provider  => shell,
    cwd       => "${::config::build_output_dir}/${::config::implementation_name}-reports",

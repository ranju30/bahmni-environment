class bahmni_jasperreports inherits bahmni_jasperreports::config {
  ...
  exec { "bahmni-jasperserver-deploy-reports" :
    provider => "shell",
    command  => "scripts/deploy.sh -j ${config::jasperHome} -p conf/${properties_file} ${deployment_log_expression}",
    path     => "${os_path}",
  ...
  exec { "bahmni-jasperserver-deploy-customserver" :
    provider => "shell",
    command  => "scripts/deployCustomJasperServer.sh ${config::jasperHome} ${deployment_log_expression}",
    path     => "${os_path}",
    cwd      => "${build_output_dir}/${implementation_name}-reports/${implementation_name}-reports-master",

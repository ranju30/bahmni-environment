class implementation_config::openelis inherits implementation_config::config {
  ...
  exec { "copyLogoToOpenelis" :
    command   => "cp ${::config::build_output_dir}/${::config::implementation_name}_config/openelis/images/labLogo.jpg ${::config::webapps_dir}/${openelis_war_file_name}/WEB-INF/reports/images",
    provider  => shell,
    path      => "${config::os_path}",
    onlyif    => "test -f ${::config::build_output_dir}/${::config::implementation_name}_config/openelis/images/labLogo.jpg"
  }

  exec { "copyLogoToOpenelisForReportConfig" :
    command   => "cp ${::config::build_output_dir}/${::config::implementation_name}_config/openelis/images/labLogo.jpg ${::config::webapps_dir}/${openelis_war_file_name}/images",
    provider  => shell,
    path      => "${config::os_path}",
    onlyif    => "test -f ${::config::build_output_dir}/${::config::implementation_name}_config/openelis/images/labLogo.jpg"
  }
  ...
  exec { "bahmni_openelis_codebase_for_liquibase_jar" :
    ...
    path     => "${config::os_path}",
    onlyif    => "test -f ${::config::build_output_dir}/${::config::implementation_name}_config/migrations/openelis/liquibase.xml",
